# ----------------------------------------------------------------------
# Code from:
# Englebert, A., Sédrick Stassin, Géraldin Nanfack, Mahmoudi, S. A., Siebert, X., Olivier Cornu, & Christophe De Vleeschouwer. (2023). 
# Explaining through Transformer Input Sampling. 
# 2023 IEEE/CVF International Conference on Computer Vision Workshops (ICCVW), 806–815. 
# https://doi.org/10.1109/iccvw60793.2023.00088
#
# https://github.com/aenglebert/Transformer_Input_Sampling/blob/main/metrics/sparseness.py
# ----------------------------------------------------------------------
...
class Sparseness:
    ...
    def __call__(self, image, saliency_map, **kwargs):
        ...
        # Rank of each item of the sorted tensor: from 1 to length+1, on the same device
        sorted_rank = torch.arange(start=1, end=length+1, step=1, device=saliency_map.device, dtype=torch.float)
        ...
        numerator = torch.sum((sorted_rank * 2 - length - 1) * sorted_saliency.values)
        denominator = length * torch.sum(sorted_saliency.values)
